...
class NodeAbosoluteInputEmbedding(nn.Module):
    def __init__(self, cfg) -> None:
        ...
        self.class_embedding = nn.Embedding(9, cfg.model.hidden_size)
...
class NodeEmbedding(nn.Module):
    ...
    def forward(self, node_mass, peak_intensity_rank, node_intensity, charge, node_class, peak_moverz):
        node = self.node_abosolute_embedding(node_intensity, peak_intensity_rank, charge, node_class)
        node_relative_pos = self.node_relative_embedding(node_mass, node_intensity, peak_moverz)
